...
# #  /* Compute number of bytes mod 64 */
    def update(self, data, _updateState=True):
        inputLen = len(data)
...
# #  /* Transform as many times as possible.
        ...
        if inputLen >= partLen:
            ...
            state = self.transform(self.buffer, state)
            i = partLen
            while i + 63 < inputLen:
                state = self.transform(data[i:i + 64], state)
                i = i + 64
            index = 0
        else:
            i = 0
...
# # */
    ...
    def digest(self):
        ...
        if index < 56:
            ...
            padLen = (120 - index)
        state = self.update(PADDING[:padLen] + bits, _updateState=False)
    ...
    def copy(self):
        ...
        newOne.count = self.count
        newOne.state = self.state
        newOne.buffer = self.buffer
        return newOne
...
def Encode(data, size):
    k = size >> 2
    res = apply(struct.pack, ("%iI" % k,) + tuple(data[:k]))
    return string.join(res, "")


def Decode(data, size):
    k = size >> 2
    res = struct.unpack("%iI" % k, data[:size])
    return list(res)
